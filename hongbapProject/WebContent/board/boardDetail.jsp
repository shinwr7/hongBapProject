<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.0/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-KyZXEAg3QhqLMpG8r+8fhAXLRk2vvoC2f3B09zVXn8CA5QIVfZOJ3BCsw2P0p/We" crossorigin="anonymous">
<style>
    *{padding: 0; margin: 0;}
    .hr{width: 800px;}
    p{font: small-caps bold 20px/1 sans-serif; color: white;}
    .p1{float: right;}
    h2{padding: 10px;font: small-caps bold 24px/1 sans-serif; color: whitesmoke;background-repeat: no-repeat; background-size: cover;background-color: rgb(212, 52, 12);width:auto; text-align: center; border-radius: 15px; }
    .table{ border: 1px solid black; border-radius: 15px;padding: 15px; background-repeat: no-repeat; background-size: cover;background-image: url("data:image/jpeg;base64,");}
    .foodca{font: small-caps bold 24px/1 sans-serif; color: white; padding: 10; margin: 10px;}
   	.buttonMain{color: white;}
   		.br{color:#A0522D;}
	.wordR{color:red;}
	.wordG{color:green;}

</style>
<meta charset="UTF-8">
<title></title>
</head>
<body>
  <div class ="container">
    <form action="/hongbapProject/boardWrite.do" method="post">
      

    <div>
    <header>
      <div class="row">

        <div class="col-xs-12"  style="background-color: wheat; border-radius: 15px;">
      <h2 >${board.bWriter}님의 게시글<p class="p1">${board.bDate}</p></h2></div>
      
    </div>
    </header>
  <div class="table" >
        <div class="row">
        <div class="foodca col-md-2" > 글제목</div>
        <div class="col-md-8"><input type="text"  style="background-color:ivory" class="form-control" value="${board.bTitle}"  readonly></div>
        <div class="col-md-1 "><P>${board.bHit}</P></div>    
    </div><br/>
      <div>
        <p class="foodca"> 글본문</p>
        <textarea cols="100" class="form-control" rows="7" value="${board.bContent}"readonly>

        </textarea>
      </div>
      <div class="row" >
        <div class="foodca"> 글쓴이</div>
        <div><input type="text" style="background-color:ivory" class="form-control"value="${nick_session }" required></div>
      </div>
    
  </div>
    <hr class="hr">
    <footer>
      <div class="button">
        <a href="/hongbapProject/boardDetail.do"><button type="submit" class="btn btn-danger buttonReq" >뒤로가기</button></a>
        <a href="/hongbapProject/boardUpdate.do"><button type="reset" class="btn btn-success buttonReset" >수정하기</button></a>

        <a href="http://localhost:8181/hongbapProject/hongbapMain/hongbapMain.jsp"><button type="button" class="btn btn-warning buttonMain" >메인화면으로 가기</button></a>

      </div>
      <hr/>
		
		
		<div class="row">
		<h6 class="text-primary text-center"><span class="br"><b>홈페이지</b> :</span> <strong>www.hongbap.com</strong></h6>
		<h6 class="text-primary text-center"><span class="br"><b>만든이</b> :</span> <strong><span class="wordR">신우람</span> <span class="wordG">조훈현</span> 서성현</strong></h6>
		</div>
		
    </footer>
  </div>
</form>
</div>
</body>
</html>